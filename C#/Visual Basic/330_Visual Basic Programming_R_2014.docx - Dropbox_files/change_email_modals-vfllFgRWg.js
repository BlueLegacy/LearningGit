  ...
  ChangeEmailWarningModal = React.createClass({
    ...
    getSSOWarning: function() {
      ...
      if (is_work_user && is_sso_user) {
        return d.div({}, _("Your %(team)s Dropbox uses single sign-on. If you change your email address you may not be able to sign in.").format({
  ...
  ChangeEmailModal = React.createClass({
    ...
    getPrompt: function() {
      ...
      if (is_paired && this.props.role === Constants.ROLE_PERSONAL) {
        ...
        if (updating) {
          parts.push(_("You'll need to verify your new email address in order to finish updating your personal email."));
        }
      } else if (is_paired && this.props.user.role === Constants.ROLE_WORK) {
        ...
        if (updating) {
          parts.push(_("You'll need to verify your new email address in order to finish updating your %(team)s email.").format({
            team: Viewer.get_viewer().team_name
          }));
        }
      } else if (updating) {
        parts.push(_("You'll need to verify your new email address in order to finish updating your email."));
      }
      if (!parts.length) {
        return '';
      }
